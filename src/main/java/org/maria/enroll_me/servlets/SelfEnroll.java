...
public class SelfEnroll extends HttpServlet {
            ...
            case "/invite":
                try {
                    ...
                    if (((System.currentTimeMillis() - meeting.getCreateDate().getTime()) < limit)
                        ...
                        String s = "<html><body><H1>Вы записанны на </H1> " +
                                "<p>" + client.getName() + "</p>" +
                                "<p>Date: " + sp[0] + "</p>" +
                                "<p>Time: " + sp[1] + "</p>" +
    ...
    private String getInviteSelf(ClientRow client, String inviteId, String times, Meeting meeting) throws ParseException {
        String sql_date = meeting.getMeetupDate();
        Logger.getLogger(DataBaseManager.class.getName()).info("Meeting data from SQL: " + sql_date);

        SimpleDateFormat parser = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date = parser.parse(sql_date);
        ...
        String start = "<html><h1>Здравствуйте, " + client.getName() + "</h1><br>\n" +
                ...
                "<label style=\"color:black;font-size:25px; for=\"datepicker\">Выберете удобное для Вас время для записи " + formattedDate + "</label>\n<br><br>" +
                "<label style=\"color:black;font-size:25px; for=\"from\">Время:</label>\n<br><br>" +
                "<select name=\"time\" id=\"time\">\n";
        String body = "";
        String[] split = times.split("/");
        System.out.println(sql_date.split(" ")[0]);
        System.out.println(times);
        for (String s : split) {
            body += "<option value=\"" + sql_date.split("  ")[0] + " " + s.trim() + "\">" + s.substring(0, 2) + "</option>\n";
        }
        String end = "</select>\n<br><br>" +
